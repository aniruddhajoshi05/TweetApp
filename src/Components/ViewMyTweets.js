...
import "./ViewMyTweets.css";

class ViewMyTweets extends React.Component {
  constructor() {
    super();
    this.tweets = [];
    this.state = {
  ...
  performAPICall = async (email) => {
    ...
    try {
      response = await (
        await fetch(
          `http://tweetapp-alb-1377564203.us-east-1.elb.amazonaws.com/api/v1.0/tweets/${email}`,
          {
            method: "GET",
            headers: {
  ...
  getMyTweets = async () => {
    const response = await this.performAPICall(localStorage.getItem("email"));
...
export default withRouter(ViewMyTweets);
